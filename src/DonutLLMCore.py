...
class LLMModel:
  ...
  def __init__(self,Path):
    if not os.path.exists(Path):
        ...
        expected_files = ['decoder.pickle', 'DoLM.pt', 'encoder.pickle']
...
class ModelTrainer:
    ...
    def SaveMyLLmModel(self,model,path,encode,decode,configData):

      #save GPT model
      # Define the directory path
      directory_path = str(os.path.join(path,'MyLLMModel'))
      ...
      #GPT Model Saved
      torch.save(model.state_dict(), os.path.join(directory_path,"DoLM.pt"))

      #save encoder
      encoder_file_path = str(os.path.join(path,'MyModel','encoder.pickle'))

      with open(encoder_file_path, 'wb') as file:
          pickle.dump(encode, file,pickle.HIGHEST_PROTOCOL)
      #save decoder
      decoder_file_path = str(os.path.join(path,'MyModel','decoder.pickle'))
      # Save the encoder to the file
      with open(decoder_file_path, 'wb') as file:
          pickle.dump(decode, file,pickle.HIGHEST_PROTOCOL)


      config_file_path = str(os.path.join(path,'MyModel','config.json'))
    ...
    def trainer(self):
        ...
        m = model.to(device)
        print(sum(p.numel() for p in m.parameters())/1e6, 'Million Parameters Model')
        ...
        for iter in range(self.max_iters):
            ...
            if iter % self.eval_interval == 0 or iter == self.max_iters - 1:
                ...
                print(f"Iteration/Epoch {iter}: Training Loss {losses['train']:.4f}, Valuation Loss {losses['val']:.4f}")
            ...
            optimizer.step()
            cwd = os.getcwd()
            torch.save(model.state_dict(), os.path.join(cwd,"DoLM.pt"))
        ...
        self.SaveMyLLmModel(m,self.SavedPath,encode,decode,configData)
